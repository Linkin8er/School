...
*   Implimented binomial distribution, cannot figure out geometric
*   implemented standard distribution, and I'm done!
*/

/*
*   03/02/2023
*   I was not done. I forgot the set operations
...
class StatsWizard{
    ...
    public void operationPicker(){
        ...
        while(wantsToContinue)
        {
            ...
            if(userChoice == 8) setOperations();
            //if(userChoice == 7) System.out.println("The geometric distribution is " + findGeometricDistribution()+"!\n");
            if(userChoice == 9) wantsToContinue = false;
        }
        System.out.println("\n\nHave a great day!");
    }
    ...
    public BigInteger factorial(int x){
        ...
        for (int i = x; i > 0; i--){
            ...
            xFactorial = xFactorial.multiply(BigInteger.valueOf(i));
        }
        return xFactorial;
    }
    ...
    public int findPermutation(int totalObjects, int selectedObjects){
        ...
        int permutations = (factorial(totalObjects).divide(factorial(totalObjects - selectedObjects))).intValue();
        return permutations;
    ...
    public long findCombination(int totalObjects, int chosingObjects){
        ...
        int combinations = BigInteger.valueOf(findPermutation(totalObjects, chosingObjects)).divide(factorial(chosingObjects)).intValue();
    ...
    //public double findGeometricDistribution(){
        //int numberOfTrials;
        //double probabilityOfSucess;
        //This needs the number of trials you want to test(x), as well as the probabilities of sucesses(p)
        //System.out.println("This function requires you enter:\nThe total number of trials you want to test(x), and the probability of sucess(p)");
        //System.out.println("First, enter the number of trials:");
        //numberOfTrials = choiceChecker(0, -100);

        //System.out.println("Now, the probability of a sucess:");
        //probabilityOfSucess = choiceChecker(0.0, 1.0);

        //double geometricDistribution = Math.pow((1-probabilityOfSucess), (numberOfTrials-1)) * probabilityOfSucess;
